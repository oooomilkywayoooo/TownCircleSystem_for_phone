...
`CREATE TABLE`・外部キー制約・想定クエリまで動作確認済みで、**管理者画面（`admin/`）は全10画面が実際にこのDBへ接続しています**（一覧表示・新規登録・編集・削除・グループ有効フラグの切替まで、ダミー配列ではなく実クエリです）。**Flutterアプリ（`mobile/`）も`api/`経由でこのDBに接続しています**（iOSシミュレータでの実機タップ検証済み）。
...
1. **画像・PDFの保存先**：`circulars.image_path` / `documents.file_path`は相対パス文字列を想定していますが、実際のアップロード処理は未実装です（登録自体はファイルなしで可能）。
2. **管理者の会員管理に新規登録がない**：会員は自分でアプリから登録する想定のため、管理者画面には意図的に会員の新規作成フォームを設けていません（README通り）。
3. **ログイントークンの永続化**：会員アプリはアプリ再起動でログイン状態が失われます（`api/README.md`参照）。

## スキーマの後追い変更

- `circulars.body`：Flutter側の回覧板詳細画面が本文を表示する設計だったにもかかわらず、当初のスキーマに列がありませんでした（管理者側の登録フォームにも本文欄がなかったため気づいていませんでした）。API接続時に気づき、`title`の次に`body TEXT NULL`を追加し、管理者の登録フォーム・APIのSELECTにも反映しています。
